...
'''
	 ...
	 *** Also it's important to protect the function signature while using decorator
'''

def superfunc(funcs):
	def inner(h):
		print 'superfunc --' , funcs,h, funcs(h)
		return funcs(h)
	return inner   
...
def protect(func):
	def maintain(g):				
		x = func(g)
		x.__name__ = g.__name__
		x.__doc__ = g.__doc__
		x.__dict__.update(g.__dict__)
		return x
	return maintain
...
# Now lets the signature - It prints the right name

print theFunc.__name__ # It prints "theFunc"
